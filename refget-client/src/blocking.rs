//! Blocking (synchronous) HTTP client for the refget API.
...
/// Uses `reqwest::blocking::Client` directly — no async runtime required.
pub struct RefgetClientBlocking {
    client: Client,
    base_url: String,
}

impl RefgetClientBlocking {
    /// Create a new blocking client with a default `reqwest::blocking::Client`.
    ...
    /// Create a new blocking client with a pre-configured `reqwest::blocking::Client`.
    ...
    fn send_optional(&self, req: RequestBuilder) -> ClientResult<Option<Response>> {
        let resp = req.send()?;
    ...
    fn send_required(&self, req: RequestBuilder) -> ClientResult<Response> {
        let resp = req.send()?;
    ...
    pub fn get_sequence(
        ...
        match self.send_optional(req)? {
            Some(resp) => Ok(Some(resp.bytes()?.to_vec())),
            None => Ok(None),
        }
    }

    /// Retrieve metadata for a sequence by digest.
    pub fn get_metadata(&self, digest: &str) -> ClientResult<Option<SequenceMetadata>> {
        let req = self.client.get(format!("{}/sequence/{digest}/metadata", self.base_url));
        match self.send_optional(req)? {
            Some(resp) => {
                let envelope: MetadataResponse = resp.json()?;
    ...
    pub fn get_sequence_service_info(&self) -> ClientResult<SequenceServiceInfo> {
        let req = self.client.get(format!("{}/sequence/service-info", self.base_url));
        let resp = self.send_required(req)?;
    ...
    pub fn get_collection_level0(&self, digest: &str) -> ClientResult<Option<String>> {
        let req = self
            .client
    ...
    pub fn get_collection_level1(&self, digest: &str) -> ClientResult<Option<SeqColLevel1>> {
        let req = self
            .client
            .get(format!("{}/collection/{digest}", self.base_url))
            .query(&[("level", "1")]);
    ...
    pub fn get_collection_level2(&self, digest: &str) -> ClientResult<Option<SeqCol>> {
        let req = self
            .client
            .get(format!("{}/collection/{digest}", self.base_url))
            .query(&[("level", "2")]);
    ...
    pub fn get_collection_raw(&self, digest: &str, level: u8) -> ClientResult<Option<Value>> {
        let req = self
            .client
            .get(format!("{}/collection/{digest}", self.base_url))
    ...
    pub fn get_attribute(&self, attr: &str, digest: &str) -> ClientResult<Option<Value>> {
        let req =
    ...
    pub fn get_seqcol_service_info(&self) -> ClientResult<Value> {
